<template>
  <div class="popup-container">
    <div class="popup-wrapper">
      <h3 class="popup-title">Edit bot</h3>
...
<script>
export default {
  props: ['name', 'description', 'image', 'date', 'id' ],
  data: () => ({
    botName: '',
    botDescription: '',
    botImage: '',
  ...
  mounted() {
    this.botName = this.name
    this.botDescription = this.description
    this.botImage = this.image
    this.botDate = this.date
    this.botId = this.id
  },
  methods: {
    formSubmit() {
      ...
      if(this.botName && this.botDescription && this.botImage && this.botDate){
        ...
        this.$emit('editedBot', this.editBot)
        alert('Success!')
      }
    },
    onFileChange(event) {
